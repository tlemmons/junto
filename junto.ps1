...
<#
.SYNOPSIS
    junto.ps1 -- unified junto workspace setup and launcher (Windows).
...
.DESCRIPTION
    ...
    Replaces junto.ps1 (replaces the old setup/init/launch scripts) for new users.
    workspace.ps1
...
# Junto local configuration -- generated by junto.ps1
...
if (Test-Path $ConfigPath) {
    ...
    Write-Error "junto: $ConfigPath not found. Run junto.ps1 interactively to complete machine setup."
    exit 1
}
...
if (-not $env:JUNTO_AGENT -or -not $env:JUNTO_PROJECT) {
    if ($Host.Name -eq 'ConsoleHost' -and -not [System.Console]::IsOutputRedirected) {
        ...
        # files, and instruct a restart. The user runs junto.ps1 again
...
> Workspace configured as ``{agent_name}@{project_name}``. Run ``junto.ps1`` again to start your session.
...
"@
        ...
        Write-Host "junto: answer the questions, then run junto.ps1 again to start your session." -ForegroundColor Cyan
    } else {
        if (-not $env:JUNTO_AGENT)   { $env:JUNTO_AGENT   = Split-Path -Leaf $cwd }
        if (-not $env:JUNTO_PROJECT) { $env:JUNTO_PROJECT = Split-Path -Leaf $cwd }
        Write-Host "junto: no identity found in $cwd, using $($env:JUNTO_AGENT)@$($env:JUNTO_PROJECT)" -ForegroundColor Yellow
